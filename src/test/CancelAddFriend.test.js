...
test('Remove Friend Success', async () => {
	...
	const strong = await driver.findElement(
		By.xpath("//span[@class='ant-menu-title-content' and contains(text(), 'Đã gửi lời mời')]")
	);
	await strong.click();
	...
	const buttonWithSpan = await driver.findElement(By.xpath("//button[span[text()='Đã gửi lời mời']]"));
	await buttonWithSpan.click();

	await driver.sleep(2000);

	const addButton = await driver.findElement(By.css('div.item--modal[value="Hủy lời mời"]'));
